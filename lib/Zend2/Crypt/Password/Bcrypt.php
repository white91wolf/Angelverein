 ...
 * @package    Zend_Crypt
 */
class Zend2_Crypt_Password_Bcrypt implements Zend2_Crypt_Password_PasswordInterface
{
    ...
    public function __construct($options = array())
    {
        if (!empty($options)) {
            if (!is_array($options)) {
                throw new Zend2Crypt_Crypt_Exception(
    ...
    {
        if (empty($this->salt)) {
            $salt = Zend2_Math_Rand::getBytes(self::MIN_SALT_SIZE);
        } else {
            $salt = $this->salt;
        }
        ...
        if (strlen($hash) <= 13) {
            throw new Zend2Crypt_Crypt_Exception('Error during the bcrypt generation');
        }
        return $hash;
    }
